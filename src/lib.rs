...
fn compile_tree(node: &Node, var_num: &mut i16) -> String {
    let rep: String = String::from(""); //IR representation

    //-compile the tree, connecting all the function calls
...
pub fn generate_ir(node: &Node) {    
    ...
    let rep: String = compile_tree(node, &mut var_num);
