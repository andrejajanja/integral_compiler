...
pub fn generate_taylor_ir(function: &String, precision_point: f64, poly_degre: usize) -> String {
    let function_infix = parse_function(function);
    let function_postfix = convert_infix_to_postfix(function_infix);

    let mut lex_processor = LexemProcessorTaylor::new(function_postfix, precision_point, poly_degre);
