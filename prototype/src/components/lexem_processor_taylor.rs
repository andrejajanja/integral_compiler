...
pub struct LexemProcessorTaylor{
    ...
    pub(crate) precision_point: f64
}

impl LexemProcessorTaylor{
    //TODO Add some custom enum to determine ops and that stuff

    pub fn new(lexems: Vec<Node>, precision_point: f64, max_power: usize) -> LexemProcessorTaylor {
    ...
    pub fn process_lexems(&mut self) {
        for elem in self.lexems.clone(){
            ...
            match self.state {
                0 => {
                    //handles multiple things at once, think about making this separate states, most notably poly to poly ops
                    self.state_0_handler();
                },
                1 => { 
                    self.state_1_handler();
                    self.state = 0;
                }
                2 => { 
                    self.state_2_handler();
                    self.state = 0;
                }
                3 => { 
                    self.state_3_handler();
                    self.state = 0;
                }
                4 => { 
                    self.state_4_handler();
                    self.state = 0;
                }
                _ => {
